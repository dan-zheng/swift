...
private:
  ...
  /// Materializes an adjoint buffer indirectly to a SIL address.
  void materializeAdjointIntoAddress(AdjointValue adjBuf, SILValue destAddress,
                                     SILLocation loc) {
    assert(adjBuf.getType() == destAddress->getType());
    assert(adjBuf.getType().isAddress());
    switch (adjBuf.getKind()) {
    /// If adjoint value is a symbolic zero, emit a call to
    /// `AdditiveArithmetic.zero`.
    case AdjointValueKind::Zero:
      emitZeroIndirect(adjBuf.getSwiftType(), destAddress, loc);
      break;
    ...
    /// buffer.
    case AdjointValueKind::Aggregate: {
      if (auto *tupTy = adjBuf.getSwiftType()->getAs<TupleType>()) {
        for (auto idx : range(adjBuf.getNumAggregateElements())) {
          ...
          materializeAdjointIntoAddress(adjBuf.getAggregateElement(idx), eltBuf,
                                        loc);
        }
      } else if (auto *structDecl =
                     adjBuf.getSwiftType()->getStructOrBoundGenericStruct()) {
        ...
        for (unsigned i = 0; fieldIt != structDecl->getStoredProperties().end();
          ...
          materializeAdjointIntoAddress(adjBuf.getAggregateElement(i), eltBuf,
                                        loc);
        }
      } else {
    ...
    /// If adjoint value is concrete, it is already materialized. Copy it to
    /// the destination address.
    case AdjointValueKind::Concrete:
      builder.createCopyAddr(loc, adjBuf.getConcreteValue(), destAddress,
                             IsNotTake, IsNotInitialization);
      break;
    }
  }

  /// Materializes an adjoint buffer indirectly into to a local `alloc_stack`.
  SILValue materializeAdjointIndirect(AdjointValue adjBuf, SILLocation loc) {
    assert(adjBuf.getType().isAddress());
    auto bufType = getRemappedTangentType(adjBuf.getType());
    // Set insertion point for local allocation builder: before the last local
    // allocation, or at the start of the pullback function's entry if no local
    // allocations exist yet.
    auto *newLocalAlloc = createFunctionLocalAllocation(
        bufType, RegularLocation::getAutoGeneratedLocation());
    // Temporarily change global builder insertion point.
    ...
    materializeAdjointIntoAddress(adjBuf, newLocalAlloc, loc);
    builder.setInsertionPoint(insertionPoint);
    return newLocalAlloc;
  }
  ...
  /// Generates code for `lhsDestAddress += rhsAddress`.
  ///
  /// Given two adjoint buffers with the same `AdditiveArithmetic`-conforming
  /// type, accumulates the rhs into the lhs and returns their sum.
  void accumulateIndirect(AdjointValue lhsDestAddress, AdjointValue rhsAddress,
                          SILLocation loc);
  ...
  Optional<AdjointValue> getAdjointProjection(SILBasicBlock *origBB,
  ...
  AdjointValue getAdjointBuffer(SILBasicBlock *origBB, SILValue originalValue) {
    ...
    // Return a symbolic zero adjoint buffer.
    auto bufType = getRemappedTangentType(originalValue->getType());
    return makeZeroAdjointValue(bufType);
  }
  ...
  void addToAdjointBuffer(SILBasicBlock *origBB, SILValue originalValue,
                          AdjointValue rhsAddress, SILLocation loc) {
...
#warning TODO: Replace `accumulateIndirect` call below (operating on `SILValue`) with logic that operates on `AdjointValue`
  ...
  void printAdjointBufferMapping() {
    ...
    for (auto pair : bufferMap) {
      ...
      tmp[origBB].insert({origBuf, adjBuf});
    }
...
public:
  ...
  void visitApplyInst(ApplyInst *ai) {
    ...
    for (auto resultIndex : applyInfo.indices.results->getIndices()) {
      ...
      case SILValueCategory::Address:
        seed =
            materializeAdjointIndirect(getAdjointBuffer(bb, origResult), loc);
        break;
      }
      args.push_back(seed);
    }
    ...
    for (unsigned i : applyInfo.indices.parameters->getIndices()) {
      ...
      if (tan->getType().isAddress()) {
        addToAdjointBuffer(bb, origArg, makeConcreteAdjointValue(tan), loc);
      } else {
        if (origArg->getType().isAddress()) {
          auto *tmpBuf = builder.createAllocStack(loc, tan->getType());
          builder.emitStoreValueOperation(loc, tan, tmpBuf,
                                          StoreOwnershipQualifier::Init);
          addToAdjointBuffer(bb, origArg, makeConcreteAdjointValue(tmpBuf),
                             loc);
  ...
  void visitLoadOperation(SingleValueInstruction *inst) {
    ...
    case SILValueCategory::Object: {
      ...
      // buffer.
      addToAdjointBuffer(bb, inst->getOperand(0),
                         makeConcreteAdjointValue(adjBuf), loc);
  ...
  void visitCopyValueInst(CopyValueInst *cvi) {
    ...
    case SILValueCategory::Object: {
      ...
      addAdjointValue(bb, cvi->getOperand(), adj, loc);
      break;
    }
    case SILValueCategory::Address: {
      auto adjDest = getAdjointBuffer(bb, cvi);
      auto destType = remapType(adjDest.getType());
      addToAdjointBuffer(bb, cvi->getOperand(), adjDest, loc);
      builder.emitDestroyAddrAndFold(loc, adjDest);
      emitZeroIndirect(destType.getASTType(), adjDest, loc);
      break;
    }
    }
  }
  ...
  void visitBeginBorrowInst(BeginBorrowInst *bbi) {
    ...
    case SILValueCategory::Object: {
      ...
      addAdjointValue(bb, bbi->getOperand(), adj, loc);
      break;
    }
    case SILValueCategory::Address: {
      auto adjDest = getAdjointBuffer(bb, bbi);
      auto destType = remapType(adjDest.getType());
      addToAdjointBuffer(bb, bbi->getOperand(), adjDest, loc);
      builder.emitDestroyAddrAndFold(loc, adjDest);
      emitZeroIndirect(destType.getASTType(), adjDest, loc);
      break;
    }
  ...
  void visitUnconditionalCheckedCastAddrInst(
    ...
    auto srcType = remapType(adjSrc.getType());
    auto destType = remapType(adjDest.getType());
    auto castBuf = builder.createAllocStack(uccai->getLoc(), srcType);
    builder.createUnconditionalCheckedCastAddr(uccai->getLoc(), adjDest,
                                               destType.getASTType(), castBuf,
                                               srcType.getASTType());
...
Optional<AdjointValue> PullbackCloner::Implementation::getAdjointProjection(
  ...
  if (auto *seai = dyn_cast<StructElementAddrInst>(originalProjection)) {
    ...
    assert(tanField && "Invalid projections should have been diagnosed");
    auto *adjProj =
        builder.createStructElementAddr(seai->getLoc(), adjSource, tanField);
  ...
  if (auto *teai = dyn_cast<TupleElementAddrInst>(originalProjection)) {
    ...
    auto *adjProj =
        builder.createTupleElementAddr(teai->getLoc(), adjSource, adjIndex);
    return makeConcreteAdjointValue(adjProj);
  }
...
void PullbackCloner::Implementation::accumulateIndirect(SILValue lhsDestAddress,
  ...
  assert(lhsDestAddress.getType() == rhsAddress.getType());
...
void PullbackCloner::Implementation::accumulateIndirect(
    AdjointValue lhsDestAddress, AdjointValue rhsAddress, SILLocation loc) {
  assert(lhsDestAddress.getType().isAddress());
  assert(lhsDestAddress.getType() == rhsAddress.getType());
#warning Implement adjoint buffer accumulation
}
