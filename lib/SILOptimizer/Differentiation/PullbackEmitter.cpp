...
Optional<TangentSpace> PullbackEmitter::getTangentSpace(CanType type) {
  return vjpEmitter.getTangentSpace(type);
}
...
SILValue PullbackEmitter::getAdjointProjection(SILBasicBlock *origBB,
  ...
  if (getPullbackInfo().hasZeroTangentVectorInitializer(originalProjection) &&
      origBB == originalProjection->getParentBlock()) {
    auto origProjType = originalProjection->getType().getASTType();
    // Handle struct aggregates
    if (auto *structDecl = origProjType->getStructOrBoundGenericStruct()) {
      auto loc = RegularLocation::getAutoGeneratedLocation();
      llvm::errs() << "PullbackEmitter::visitStructElementAddrInst\n";
      llvm::errs() << "ORIGINAL PROJECTION!\n";
      originalProjection->dump();
      auto structTanType = getRemappedTangentType(originalProjection->getType());
      auto *tangentVectorDecl = structTanType.getASTType().getStructOrBoundGenericStruct();
      auto zeroTanBuffer = createFunctionLocalAllocation(structTanType, loc);
      unsigned adjIndex = 0;
      for (auto pair : enumerate(structDecl->getStoredProperties())) {
        auto *field = pair.value();
        auto fieldIndex = pair.index();
        // TODO: Remap type? Check noDerivative?
        ...
        if (field->getAttrs().hasAttribute<NoDerivativeAttr>())
          continue;
        if (!getTangentSpace(field->getValueInterfaceType()->getCanonicalType()))
          continue;
        auto *zeroTanInitField =
            getPullbackInfo().lookUpZeroTangentVectorInitializer(originalProjection, adjIndex);
        auto zeroTanInitVal = getPullbackStructElement(origBB, zeroTanInitField);
        // NOTE: WE DON'T NEED TO REABSTRACT IT BACK, JUST CHECK HOW TO APPLY IT
#if 0
        if (vjpEmitter.originalZeroTangentVectorInitializerTypes.count(field)) {
          auto actualType = vjpEmitter.originalZeroTangentVectorInitializerTypes[field];
          SILOptFunctionBuilder fb(getContext().getTransform());
          zeroTanInitVal = reabstractFunction(
              builder, fb, loc, zeroTanInitVal, actualType,
              [this](SubstitutionMap subs) -> SubstitutionMap {
                return this->remapSubstitutionMap(subs);
              });
        }
#endif
        auto zeroTanInitValType = zeroTanInitVal->getType().castTo<SILFunctionType>();
        auto zeroTanInitResult = zeroTanInitValType->getResults().back();
        ...
        auto zeroTanProj = builder.createStructElementAddr(loc, zeroTanBuffer, tanField);
        llvm::errs() << "ZERO TAN INIT RESULT\n";
        zeroTanInitVal->dump();
        zeroTanInitVal->getType().dump();
        zeroTanInitResult.dump();
        if (zeroTanInitResult.isFormalIndirect()) {
          llvm::errs() << "FORMAL INDIRECT\n";
          builder.createApply(loc, zeroTanInitVal, {}, {zeroTanProj});
        } else {
          llvm::errs() << "FORMAL DIRECT\n";
          auto zeroTan = builder.createApply(loc, zeroTanInitVal, {}, {});
          builder.emitStoreValueOperation(loc, zeroTan, zeroTanProj, StoreOwnershipQualifier::Init);
          builder.emitDestroyValueOperation(loc, zeroTan);
        }
        llvm::errs() << "STRUCT ELEMENT ADDR ZERO TAN INIT\n";
        builder.emitDestroyValueOperation(loc, zeroTanInitVal);
        ++adjIndex;
      }
      return zeroTanBuffer;
    }
#if 0
    auto *zeroTanInitField =
        getPullbackInfo().lookUpZeroTangentVectorInitializer(originalProjection);
    auto zeroTanInitVal = getPullbackStructElement(origBB, zeroTanInitField);
    auto loc = RegularLocation::getAutoGeneratedLocation();
    auto zeroTanBuffer = createFunctionLocalAllocation(getRemappedTangentType(originalProjection->getType()), loc);
    auto zeroTan = builder.createApply(loc, zeroTanInitVal, {}, {zeroTanBuffer});
    llvm::errs() << "ORIG PROJ\n";
    originalProjection->dump();
    llvm::errs() << "STRUCT ELEMENT ADDR ZERO TAN INIT\n";
    zeroTan->dumpInContext();
    zeroTan->getType().dump();
    builder.emitDestroyValueOperation(loc, zeroTanInitVal);
#endif
  }
...
#if 0
    if (getPullbackInfo().hasZeroTangentVectorInitializer(seai->getOperand())) {
      if (seai->getParent() == origBB) {
        auto *zeroTanInitField =
            getPullbackInfo().lookUpZeroTangentVectorInitializer(seai->getOperand());
        auto zeroTanInitVal = getPullbackStructElement(origBB, zeroTanInitField);
        auto zeroTanBuffer = createFunctionLocalAllocation(getRemappedTangentType(seai->getType()), seai->getLoc());
        auto zeroTan = builder.createApply(seai->getLoc(), zeroTanInitVal, {}, {zeroTanBuffer});
        // accumulateIndirect(<#SILValue lhsDestAccess#>, <#SILValue rhsAccess#>, <#SILLocation loc#>)
        llvm::errs() << "STRUCT ELEMENT ADDR ZERO TAN INIT\n";
        zeroTan->dumpInContext();
        zeroTan->getType().dump();
        builder.emitDestroyValueOperation(seai->getLoc(), zeroTanInitVal);
        // assert(false);
        return zeroTanBuffer;
      }
    }
...
SILValue &PullbackEmitter::getAdjointBuffer(SILBasicBlock *origBB,
  ...
  // allocations exist yet.
  auto loc = RegularLocation::getAutoGeneratedLocation();
  auto *newBuf = createFunctionLocalAllocation(bufObjectType, loc);
...
#if 0
  if (auto *zeroTanInitDecl =
      getPullbackInfo().lookUpZeroTangentVectorInitializer(originalBuffer)) {
    llvm::errs() << "ZERO TAN INIT!\n";
    zeroTanInitDecl->dumpRef(); llvm::errs() << "\n";
    auto zeroTanInitVal = getPullbackStructElement(origBB, zeroTanInitDecl);
    zeroTanInitVal->dump();
    auto zeroTan = builder.createApply(loc, zeroTanInitVal, {}, {});
    builder.emitStoreValueOperation(loc, zeroTan, newBuf, StoreOwnershipQualifier::Init);
    builder.setInsertionPoint(insertionPoint);
    return (insertion.first->getSecond() = newBuf);
  }
#endif
  emitZeroIndirect(bufObjectType.getASTType(), newBuf, loc);
  builder.setInsertionPoint(insertionPoint);
  return (insertion.first->getSecond() = newBuf);
}
...
bool PullbackEmitter::run() {
  ...
  llvm::errs() << "ADJOINT BUFFER MAPPING\n";
  for (auto pair : bufferMap) {
    auto pair2 = std::get<0>(pair);
    auto adjBuf = std::get<1>(pair);
    auto *bb = std::get<0>(pair2);
    auto origValue = std::get<1>(pair2);
    llvm::errs() << "ORIG BB " << bb->getDebugID() << "\n";
    llvm::errs() << "ORIG BUF: " << origValue;
    llvm::errs() << "ADJ BUF: " << adjBuf;
  }
...
void PullbackEmitter::visitStructExtractInst(StructExtractInst *sei) {
  ...
  llvm::errs() << "PullbackEmitter::visitStructExtractInst: " << *sei << "\n";
  switch (av.getKind()) {
  case AdjointValueKind::Zero: {
    llvm::errs() << "FOUND ZERO\n";
#if 0
    addAdjointValue(bb, sei->getOperand(),
                    makeZeroAdjointValue(tangentVectorSILTy), sei->getLoc());
#endif
    auto *zeroTanInitField =
        getPullbackInfo().lookUpZeroTangentVectorInitializer(sei->getOperand(), sei->getFieldNo());
    auto zeroTanInitVal = getPullbackStructElement(bb, zeroTanInitField);
    zeroTanInitVal->dumpInContext();
    addAdjointValue(bb, sei->getOperand(),
                    makeConcreteAdjointValue(zeroTanInitVal), sei->getLoc());
    break;
  }
  case AdjointValueKind::Concrete:
  case AdjointValueKind::Aggregate: {
    llvm::errs() << "FOUND CONCRETE/AGGREGATE\n";
    av.dump();
    SmallVector<AdjointValue, 8> eltVals;
    for (auto pair : enumerate(tangentVectorDecl->getStoredProperties())) {
      auto *field = pair.value();
      auto index = pair.index();
      auto *zeroTanInitField =
          getPullbackInfo().lookUpZeroTangentVectorInitializer(sei->getOperand(), index);
      auto zeroTanInitVal = getPullbackStructElement(bb, zeroTanInitField);
      if (field == tanField) {
        eltVals.push_back(av);
      } else {
        auto zeroTan = builder.createApply(sei->getLoc(), zeroTanInitVal, {}, {});
        eltVals.push_back(makeConcreteAdjointValue(zeroTan));
        recordTemporary(zeroTan);
      }
      if (!blockTemporaries[zeroTanInitVal->getParentBlock()].count(zeroTanInitVal))
        recordTemporary(zeroTanInitVal);
      // builder.emitDestroyValueOperation(sei->getLoc(), zeroTanInitVal);
    }
    getPullback().dump();
    addAdjointValue(bb, sei->getOperand(),
                    makeAggregateAdjointValue(tangentVectorSILTy, eltVals),
    ...
    // Replace a call to a function with a call to its pullback.
    auto &nestedApplyInfo = getContext().getNestedApplyInfo();
    auto applyInfoLookup = nestedApplyInfo.find(sei);
    // If no `NestedApplyInfo` was found, then this task doesn't need to be
    // differentiated.
    if (applyInfoLookup == nestedApplyInfo.end()) {
      // Must not be active.
    ...
    auto zeroTan = builder.createApply(sei->getLoc(), zeroTanInitVal, {}, {});
    addAdjointValue(bb, sei->getOperand(), makeConcreteAdjointValue(zeroTan),
                    sei->getLoc());
    builder.emitDestroyValueOperation(sei->getLoc(), zeroTanInitVal);
    // builder.emitDestroyValueOperation(sei->getLoc(), zeroTan);
    recordTemporary(zeroTan);
#endif
