...
private:
  ...
  enum IsInitialized_t : bool { NotInitialized, Initialized };
  /// Mapping from tangent buffers to their initialization info. Used to keep
  /// track of whether a tangent buffer is initalized of not.
  llvm::DenseMap<SILValue, IsInitialized_t> tangentBufferInitializationInfo;
  ...
  SILValue getTangentBuffer(SILBasicBlock *origBB, SILValue originalBuffer,
                            InitializationRequired_t initializationRequired =
                                InitializationNotRequired) {
...
#define CLONE_AND_EMIT_TANGENT(INST, ID)                                       \
  ...
  CLONE_AND_EMIT_TANGENT(Load, li) {
    ...
    auto loc = li->getLoc();
    auto tanBuf =
        getTangentBuffer(bb, li->getOperand(), InitializationRequired);
  ...
  CLONE_AND_EMIT_TANGENT(LoadBorrow, lbi) {
    ...
    auto loc = lbi->getLoc();
    auto tanBuf =
        getTangentBuffer(bb, lbi->getOperand(), InitializationRequired);
  ...
  CLONE_AND_EMIT_TANGENT(CopyAddr, cai) {
    ...
    auto tanDest =
        getTangentBuffer(bb, cai->getDest(), InitializationNotRequired);
  ...
  CLONE_AND_EMIT_TANGENT(UnconditionalCheckedCastAddr, uccai) {
    ...
    auto tanSrc = getTangentBuffer(bb, uccai->getSrc(), InitializationRequired);
    auto tanDest =
        getTangentBuffer(bb, uccai->getDest(), InitializationNotRequired);
  ...
  CLONE_AND_EMIT_TANGENT(BeginAccess, bai) {
    ...
    setTangentBuffer(bb, bai, tanDest,
                     /*isInitialized*/ tangentBufferInitializationInfo[tanSrc]);
  }
  ...
  CLONE_AND_EMIT_TANGENT(StructElementAddr, seai) {
    ...
    setTangentBuffer(
        bb, seai, tangentInst,
        /*isInitialized*/ tangentBufferInitializationInfo[tanOperand]);
  }
  ...
  CLONE_AND_EMIT_TANGENT(TupleElementAddr, teai) {
    ...
    setTangentBuffer(teai->getParent(), teai, tanBuf, Initialized);
  }
...
#undef CLONE_AND_EMIT_TANGENT
  ...
  void emitTangentForApplyInst(ApplyInst *ai, SILAutoDiffIndices applyIndices,
    ...
    for (auto indRes : ai->getIndirectSILResults())
      diffArgs.push_back(
          getTangentBuffer(bb, indRes, InitializationNotRequired));
    ...
    auto *differentialCall =
        diffBuilder.createApply(loc, differential, SubstitutionMap(), diffArgs,
                                /*isNonThrowing*/ false);
    // Differential indirect results are now initialized.
    for (auto indRes : ai->getIndirectSILResults()) {
      auto tanIndRes = getTangentBuffer(bb, indRes, InitializationNotRequired);
      setTangentBufferInitializationInfo(tanIndRes, Initialized);
    }
  ...
  void emitReturnInstForDifferential() {
    ...
    for (auto resultIndex : getIndices().results->getIndices()) {
      ...
      // Handle only original formal direct results.
      if (resultIndex >= origFnTy->getNumResults() ||
          getTangentValueCategory(origResult->getType()) !=
  ...
  */
  ...
  for (auto index : range(diffParamArgs.size())) {
    ...
    if (diffArg->getType().isAddress()) {
      setTangentBuffer(origEntry, origArg, diffArg, Initialized);
    } else {
