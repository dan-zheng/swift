    ...
    for (auto &inst : origBB) {
      if (auto *ai = dyn_cast<ApplyInst>(&inst)) {
        ...
        if (isa<StructElementAddrInst>(dest)) {
          ...
          llvm::errs() << "LINEAR MAP INFO FOUND BASE!\n";
          base->dump();

          auto tanStructType = sei->getOperand()
              ->getType().getASTType()->getAutoDiffTangentSpace(LookUpConformanceInModule(moduleDecl))->getCanonicalType();
          tanStructType->dump();

          addZeroTangentVectorInitializerDecl(&origBB, base, tanStructType);
        }
      }
#if 0
