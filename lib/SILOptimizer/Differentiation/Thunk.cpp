...
static SILValue emitManagedBeginBorrow(SILBuilder &builder, SILLocation loc, SILValue v) {
...
static void forwardFunctionArgumentsConvertingOwnership(
    ...
    SmallVectorImpl<std::pair<SILValue, CleanupOperation>> &argumentsToCleanup) {
  auto &function = builder.getFunction();
  ...
  for (auto index : indices(managedArgs)) {
    ...
    if (fromParam.isGuaranteed() && !toParam.isGuaranteed()) {
      llvm::errs() << "GUARANTEED TO OWNED, ARG OWNERSHIP KIND: " << (unsigned)arg->getOwnershipKind() << "\n";
...
SILFunction *getOrCreateReabstractionThunk(SILOptFunctionBuilder &fb,
  ...
  // forwardFunctionArguments(builder, loc, <#CanSILFunctionType fTy#>, <#ArrayRef<SILValue> managedArgs#>, <#SmallVectorImpl<SILValue> &forwardedArgs#>)
...
// #if 0
  ...
  llvm::errs() << "THUNK ARGUMENTS: " <<thunk->getArgumentsWithoutIndirectResults().drop_back().size() <<  "\n";
  ...
  forwardFunctionArgumentsConvertingOwnership(builder, loc, fromType, toType,
                                              thunk->getArgumentsWithoutIndirectResults().drop_back(), forwardedArgs, localAllocations, argumentsToCleanup);
