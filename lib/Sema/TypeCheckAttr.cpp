...
/// Returns the diagnostics-friendly name of the given accessor kind.
static std::string getAccessorDiagnosticsName(AccessorKind kind) {
  // Get and set accessors have standard user-facing names.
  if (kind == AccessorKind::Get)
    return "getter";
  if (kind == AccessorKind::Set)
    return "setter";
  // Otherwise, return "'<kind>' accessor".
  return "'" + getAccessorLabel(kind).str() + "' accessor";
}
  ...
  if (validCandidates.empty()) {
    ...
    for (auto invalidCandidatePair : invalidCandidates) {
      ...
      case AbstractFunctionDeclLookupError::Kind::CandidateMissingAccessor: {
        auto accessorKind = maybeAccessorKind.getValueOr(AccessorKind::Get);
        auto accessorName = getAccessorDiagnosticsName(accessorKind);
        diags.diagnose(invalidCandidate,
               diag::autodiff_attr_original_decl_missing_accessor,
               getAccessorDiagnosticsName(accessorKind));
#if 0
        diags.diagnose(invalidCandidate,
               diag::autodiff_attr_original_decl_missing_accessor,
               invalidCandidate->getDescriptiveKind());
#endif
        break;
      }
  ...
  if (originalName.AccessorKind.hasValue()) {
    if (*originalName.AccessorKind != AccessorKind::Get &&
      ...
      diags.diagnose(originalName.Loc,
                     ...
                     getAccessorDiagnosticsName(*originalName.AccessorKind));
      return true;
    }
  }
