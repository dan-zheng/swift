...
static bool canDeriveTangentVector(NominalTypeDecl *nominal, DeclContext *DC) {
  ...
  auto invalidTangentDecls = llvm::partition(tangentDecls, [&](ValueDecl *v) {
    return isValidTangentVectorCandidate(v);
  });
...
static bool canDeriveZeroTangentVectorInitializer(NominalTypeDecl *nominal,
                                                  DeclContext *DC) {
  auto &C = nominal->getASTContext();

  // Check if a valid `TangentVector` type witness exists.
  ...
  // If the `TangentVector` type witness exists and conforms to
  // `AdditiveArithmetic`, return true.
  //
  // It is guaranteed that `zeroTangentVectorInitializer` can be synthesized (as
  // `{ TangentVector.zero }` as a fallback).
  auto *addArithProto = C.getProtocol(KnownProtocolKind::AdditiveArithmetic);
  if (TypeChecker::conformsToProtocol(tangentVectorType, addArithProto, DC))
    return true;

  // Otherwise, check whether `TangentVector` derivation is possible.
...
// #if 0
  ...
  if (requirement->getBaseName() == C.Id_TangentVector ||
      requirement->getBaseName() == C.Id_move)
    return canDeriveTangentVector(nominal, DC);
  if (requirement->getBaseName() == C.Id_zeroTangentVectorInitializer)
...
deriveBodyDifferentiable_zeroTangentVectorInitializer(
  ...
  bool canPerformMemberwiseDerivation = [&]() -> bool {
    // Memberwise derivation is possible only for struct types.
