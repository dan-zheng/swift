...
Optional<TangentSpace> TangentSpaceRequest::evaluate(Evaluator &evaluator,
                                                     Type type,
                                                     ModuleDecl *module) const {
  return type->getAutoDiffTangentSpace(LookUpConformanceInModule(module));
}
...
TangentStoredPropertyRequest::evaluate(Evaluator &evaluator,
  ...
  if (!originalFieldTanType->isEqual(tanField->getType())) {
    return TangentPropertyInfo(
        TangentPropertyInfo::Error::Kind::TangentPropertyIncompatibleType,
