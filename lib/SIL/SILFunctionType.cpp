...
// #endif
  ...
  SmallVector<GenericTypeParamType *, 4> outerSubstGenericParams(
      substMap.getGenericSignature()->getGenericParams().begin(),
      substMap.getGenericSignature()->getGenericParams().end());
  ...
  case AutoDiffDerivativeFunctionKind::JVP: {
    ...
    for (auto &param : diffParams) {
      ...
      if (!paramTan->getCanonicalType()->hasArchetype()) {
    ...
    if (!inoutParam || !isWrtInoutParameter) {
      ...
      if (!resultTan->getCanonicalType()->hasArchetype()) {
