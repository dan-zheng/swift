...
// expected-note @+1 {{candidate does not have expected type '<T where T : Differentiable, T == T.TangentVector> (T) -> T'}}
...
// expected-note @+1 {{candidate does not have expected type '<T where T : Differentiable, T == T.TangentVector> (Struct<T>) -> (Float) -> Struct<T>'}}
struct Struct<T> {}
...
struct StoredProperty: Differentiable & AdditiveArithmetic {
  // expected-note @+1 {{candidate does not have expected type '(StoredProperty) -> () -> StoredProperty'}}
  var stored: Float
